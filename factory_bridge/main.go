...
func main() {
    ...
    for _, dir := range []string{commandsDir, filepath.Join(cfg.BridgeRoot, "02_RESULTS"), filepath.Join(cfg.BridgeRoot, "03_ARCHIVE")} {
        if err := os.MkdirAll(dir, 0o755); err != nil {
            fmt.Fprintln(os.Stderr, err)
            os.Exit(2)
        }
    }
    fmt.Printf("FactoryBridge %s. Waiting for commands in 01_COMMANDS ...\n", bridgeVersion)
    ...
    for {
        ...
        if err == nil {
            for _, entry := range entries {
                ...
                if err := processOne(cfg, filepath.Join(commandsDir, entry.Name()), osRunner{}); err != nil {
